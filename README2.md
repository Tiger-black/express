...
二、中间件
    使用中间件
    Express是一个自身功能极简，完全是由路由和中间件构成一个的web开发框架：从本质上来说，一个 Express 应用就是在调用各种中间件。
    ...
    Express 应用可使用如下几种中间件：
        ...
        - 内置中间件
        - 第三方中间件
    使用可选则挂载路径，可在应用级别或路由级别装载中间件。另外你还可以同时装在一系列中间件函数，从而在一个挂载点上创建一个子中间件栈。

    1、应用级中间件
        应用级中间件绑定到 app 对象 使用 app.use() 和 app.METHOD()， 
        （1）其中， METHOD 是需要处理的 HTTP 请求的方法，例如 GET, PUT, POST 等等，全部小写。例如：
            var app = express();
        ...
        （3）作为中间件系统的路由句柄，使得为路径定义多个路由成为可能。
        ...
        （4）如果需要在中间件栈中跳过剩余中间件，调用 next('route') 方法将控制权交给下一个路由。
    ...
    4、内置中间件
        ...
        类型	        缺省值             属性	            描述	                
        String	    “ignore”        dotfiles        是否对外输出文件名以点（.）开头的文件。可选值为 “allow”、“deny” 和 “ignore”
        Boolean	    true            etag	        是否启用 etag 生成	
        Array	    []              extensions	    设置文件扩展名备份选项	
        Mixed	    “index.html”    index	        发送目录索引文件，设置为 false 禁用目录索引。	
        Boolean	    true            lastModified	设置 Last-Modified 头为文件在操作系统上的最后修改日期。可能值为 true 或 false。	
        Number	    0               maxAge	        以毫秒或者其字符串格式设置 Cache-Control 头的 max-age 属性。	
        Boolean	    true            redirect	    当路径为目录时，重定向至 “/”。	
        Function	                setHeaders	    设置 HTTP 头以提供文件的函数。	
    ...
    5、第三方中间件
        ...
        请参考 第三方中间件 获取 Express 中经常用到的第三方中间件列表。   http://www.expressjs.com.cn/resources/middleware.html
